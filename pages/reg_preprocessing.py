...
def write(state):
    ...
    if state.df is not None:
        ...
        advance_preprocess = st.checkbox('Use advance preprocess?')
            ...
            if button_run:
                with st.spinner("Preprocessing..."):
                    if advance_preprocess:
                        ...
                        setup(data=df,target=target_column, preprocess=True,html=False,silent=True)

                state.log_history["set_up"] = pull(True)
                state.is_set_up = True
                state.best = None
                
                with st.spinner("Loading transformed data info..."):
                    AgGrid(state.log_history["set_up"].data)

        
        st.markdown("""---""")
        st.subheader("Estimate all machine learning models for Regression:")
        ...
        try:
            if button_compare:
                if 'is_set_up' not in st.session_state:
